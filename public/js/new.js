...
const header = getId("header"),
  ...
  scale = parallax && parallax.hasAttribute("data-scale") ? parallax.getAttribute("data-scale") : 0.05,
  ...
  decodeFeatureImage = () => {
    ...
    e &&
      decodeImage(e, () => {
        TweenMax.to(e, 0.5, { autoAlpha: 1, delay: 0.3, ease: Linear.easeNone });
      });
  },
  decodeImages = () => {
  ...
  animateIn = () => {
    if ((TIMELINES.globalIn.add(() => getId("wrapper").classList.add("visible"), 0.1), TIMELINES.globalIn.fromTo("#scrollme", 1.5, { autoAlpha: 0 }, { autoAlpha: 1, yoyo: !0, repeat: -1, ease: Expo.easeInOut }, 1).fromTo("#scrollme", 3, { y: -40 }, { y: -30, repeat: -1, ease: Expo.easeInOut }, 1).restart(), stats && supportsIO && (TIMELINES.count.fromTo("#stats .number", 0.4, { autoAlpha: 0 }, { autoAlpha: 1 }, 0), TIMELINES.count.fromTo("#stats .label", 0.5, { autoAlpha: 0 }, { autoAlpha: 1 }, 0.5), queryAll("#stats .stat").length))) {
...
const limit = header.hasAttribute("data-limit") ? header.getAttribute("data-limit") : 0.45,
  headerStates = () => {
    if (!hasBackground) {
  ...
  inViewport = (e, t, a) => {
    TweenMax.fromTo(e, 0.85, { autoAlpha: 0 }, { autoAlpha: 1, ease: Power3.easeOut, delay: a }), TweenMax.fromTo(e, 1.1, { y: t }, { y: 0, ease: Power4.easeOut, delay: a, force3D: !0 });
  },
  addIO = () => {
    ...
    const e = deviceSettings.isMobile ? 30 : 80,
      ...
      a = [],
      n = queryAll(".animate");
    n.forEach((e) => a.push({ el: e, y: 0, ratio: 0, animated: !1 })), n.forEach((e) => TweenMax.set(e, { autoAlpha: 0 }));
    ...
    open
      ? (getId("hamburger").setAttribute("aria-expanded", "false"),
        ...
        e
          ...
          .to("#menu-close", 0.5, { autoAlpha: 0, onComplete: () => (closed = !1) }, 0)
          .to("#menu", 1, { width: 0, ease: Expo.easeOut }, 0)
          .to("#menu-navigation", 0.5, { autoAlpha: 0 }, 0)
          .to(".primary, .secondary", 0.1, { autoAlpha: 0, overwrite: !0, ease: Linear.easeNone }, 0))
      : (getId("hamburger").setAttribute("aria-expanded", "true"),
        ...
        e
          ...
          .to("#menu", 1, { width: innerWidth <= 600 ? 290 : 350, ease: Expo.easeOut }, 0)
          .to("#menu-navigation", 0.5, { autoAlpha: 1 }, 0)
          .staggerFromTo(innerWidth <= 960 ? ".primary, .secondary" : ".secondary", 1, { y: 15, autoAlpha: 0 }, { y: 0, autoAlpha: 1, ease: Expo.easeOut }, 0.05, 0.1)
          .fromTo("#menu-background", 1, { skewX: 5 }, { skewX: 0, ease: Expo.easeOut }, 0)),
      e.restart();
  };
let modalState = !1;
const modal = getId("modal"),
  ...
  openModal = ({ url: e = "sOpMrVnjYeY", title: t = "" }) => {
    ...
    const a = new TimelineMax({ paused: !0 }).set(modal, { autoAlpha: 1, display: "block" }, 0);
    a.to(queryAll(".section"), 1.25, { y: -0.25 * innerHeight, ease: Quint.easeInOut }, 0),
      a
        .staggerFromTo([modal, modalchild], 1.25, { cycle: { y: [innerHeight, -0.5 * innerHeight] } }, { y: 0, ease: Quint.easeInOut }, 0, 0)
        .fromTo(modalclose, 1, { rotation: -90, autoAlpha: 0 }, { rotation: 0, autoAlpha: 1, ease: Expo.easeOut }, 0)
        .add(() => {
          ...
          t.on("ready", (e) => {
            t.play(), assign(t.elements.container.style, { position: "absolute", top: 0, left: 0, width: "100%", height: "100%" });
          });
        }, 0.75)
        .restart();
  },
  closeModal = () => {
    ...
    a.fromTo(queryAll(".section"), 1, { y: 0.5 * innerHeight }, { y: 0, ease: Quint.easeInOut, clearProps: "transform" }, 0),
      a
        .staggerTo([e, modalchild], 1, { cycle: { y: [-innerHeight, 0.5 * innerHeight] }, ease: Quint.easeInOut }, 0, 0)
